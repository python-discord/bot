    ...
    async def test_set_slowmode_no_channel(self) -> None:
        ...
        test_cases = (
            (
                ...
                23,
                True,
                f"{Emojis.check_mark} The slowmode delay for #helpers is now 23 seconds.",
            ),
            (
                "mods",
                76526,
            ...
            (
                ...
                97,
                True,
                f"{Emojis.check_mark} The slowmode delay for #admins is now 1 minute and 37 seconds.",
            ),
        )
        ...
        for channel_name, seconds, edited, result_msg in test_cases:
            with self.subTest(
                ...
                await self.cog.set_slowmode(
                    self.cog, self.ctx, None, relativedelta(seconds=seconds)
                )

                if edited:
                    self.ctx.channel.edit.assert_awaited_once_with(
                        slowmode_delay=float(seconds)
                    )
                else:
    ...
    async def test_set_slowmode_with_channel(self) -> None:
        ...
        test_cases = (
            (
                ...
                12,
                True,
                f"{Emojis.check_mark} The slowmode delay for #bot-commands is now 12 seconds.",
            ),
            (
                "mod-spam",
                21,
                True,
                f"{Emojis.check_mark} The slowmode delay for #mod-spam is now 21 seconds.",
            ),
            (
                "admin-spam",
                4323598,
        ...
        for channel_name, seconds, edited, result_msg in test_cases:
            with self.subTest(
                ...
                await self.cog.set_slowmode(
                    self.cog, self.ctx, text_channel, relativedelta(seconds=seconds)
                )

                if edited:
                    text_channel.edit.assert_awaited_once_with(
                        slowmode_delay=float(seconds)
                    )
                else:
