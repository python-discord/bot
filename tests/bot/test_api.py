...
class APIClientTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_response_code_error_string_representation_default_initialization(self):
        ...
        error = api.ResponseCodeError(response=self.error_api_response)
        self.assertEqual(
            str(error), f"Status: {self.error_api_response.status} Response: "
        )
    ...
    def test_response_code_error_string_representation_with_nonempty_response_json(
        self,
    ):
        """Test the string representation of `ResponseCodeError` initialized with json."""
        json_data = {"hello": "world"}
        error = api.ResponseCodeError(
            response=self.error_api_response, response_json=json_data
        )
        self.assertEqual(
            str(error),
            f"Status: {self.error_api_response.status} Response: {json_data}",
        )
    ...
    def test_response_code_error_string_representation_with_nonempty_response_text(
        self,
    ):
        """Test the string representation of `ResponseCodeError` initialized with text."""
        text_data = "Lemon will eat your soul"
        error = api.ResponseCodeError(
            response=self.error_api_response, response_text=text_data
        )
        self.assertEqual(
            str(error),
            f"Status: {self.error_api_response.status} Response: {text_data}",
        )
