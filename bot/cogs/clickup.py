...
class ClickUp:
    ...
    async def on_ready(self):
        ...
        if "err" in result:
            log.error(
                f"Failed to get ClickUp lists: `{result['ECODE']}`: {result['err']}"
            )
        else:
    ...
    async def tasks_command(
        self, ctx: Context, status: str = None, task_list: str = None
    ):
        """
        ...
        if "err" in result:
            ...
            if task is None:
                ...
                if task.get("assignees"):
                    assignees = ", ".join(
                        user["username"] for user in task["assignees"]
                    )
    ...
    async def team_command(self, ctx: Context):
        ...
        if "err" in result:
            log.error(
                "ClickUp responded to the team request with an error!\n"
                f"error code: '{result['ECODE']}'\n"
                f"error: {result['err']}"
            )
            embed = Embed(
    ...
    async def lists_command(self, ctx: Context):
        ...
        if "err" in result:
            log.error(
                "ClickUp responded to the lists request with an error!\n"
                f"error code: '{result['ECODE']}'\n"
                f"error: {result['err']}"
            )
            embed = Embed(
            ...
            log.debug(
                f"{ctx.author} requested a list of all ClickUp lists. Preparing the list..."
            )
            embed = Embed(colour=Colour.blurple())
            ...
            for project in result["projects"]:
                ...
                embed.add_field(
                    name=f"{project['name']} ({project['id']})", value=lists
                )
    ...
    async def open_command(self, ctx: Context, task_list: str, title: str):
        ...
        if task_list in self.lists:
            ...
            log.warning(
                f"{ctx.author} tried to open a new task on ClickUp, "
        ...
        response = await self.bot.http_session.post(
            CREATE_TASK_URL.format(list_id=task_list),
            headers=HEADERS,
            json={"name": title, "status": "Open"},
        )
        result = await response.json()
        ...
        if "err" in result:
            ...
            log.debug(
                f"{ctx.author} opened a new task on ClickUp: \n"
                f"{task_list} - #{task_id}"
            )
    ...
    @command(
        name="clickup.set_status()",
        aliases=["clickup.set_status", "set_status", "set_task_status"],
    )
    @with_role(MODERATOR_ROLE, ADMIN_ROLE, OWNER_ROLE, DEVOPS_ROLE, CONTRIBUTOR_ROLE)
    async def set_status_command(self, ctx: Context, task_id: str, status: str):
        ...
        if status.lower() not in STATUSES:
            log.warning(
                f"{ctx.author} tried to update a task on ClickUp, but '{status}' is not a known status."
            )
            embed.description = f"Unknown status: {status}"
            embed.colour = Colour.red()
        else:
            response = await self.bot.http_session.put(
                EDIT_TASK_URL.format(task_id=task_id),
                headers=HEADERS,
                json={"status": status},
            )
            result = await response.json()
