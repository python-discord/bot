...
class Clean:
    ...
    You can delete messages sent by a specific user, messages sent by bots, all messages, or
    messages that match a specific regular expression.

    The deleted messages are saved and uploaded to the database via an API endpoint, and a URL is
    returned which can be used to view the messages in the Discord dark theme style.
    """
    ...
    async def _clean_messages(
        ...
        def predicate_regex(message: Message):
            """
            Returns True if the regex provided in the _clean_messages matches the message content or
            any embed attributes the message may have.
            """
    ...
    async def clean_user(self, ctx: Context, user: User, amount: int = 10):
        """
        Delete messages posted by the provided user, and stop cleaning after traversing `amount`
        messages.
        """
    ...
    async def clean_all(self, ctx: Context, amount: int = 10):
        """
        Delete all messages, regardless of poster, and stop cleaning after traversing `amount`
        messages.
        """
    ...
    async def clean_bots(self, ctx: Context, amount: int = 10):
        """
        Delete all messages posted by a bot, and stop cleaning after traversing `amount` messages.
        """
    ...
    async def clean_regex(self, ctx: Context, regex, amount: int = 10):
        """
        Delete all messages that match a certain regex, and stop cleaning after traversing `amount`
        messages.
        """
