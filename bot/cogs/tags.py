...
class TagNameConverter(Converter):
    ...
    async def convert(ctx: Context, tag_name: str):
        ...
        if ascii(tag_name)[1:-1] != tag_name:
            log.warning(
                f"{ctx.author} tried to put an invalid character in a tag name. "
        ...
        # The tag name is a number of some kind, we don't allow that.
            ...
            log.warning(
                f"{ctx.author} tried to create a tag with a digit as its name. "
                "Rejecting the request."
            )
        ...
        # The tag name is longer than 127 characters.
            ...
            log.warning(
                f"{ctx.author} tried to request a tag name with over 127 characters. "
                "Rejecting the request."
            )
...
class Tags:
    ...
    async def get_tag_data(self, tag_name=None) -> dict:
        ...
        response = await self.bot.http_session.get(
            SITE_API_TAGS_URL, headers=self.headers, params=params
        )
    ...
    async def post_tag_data(self, tag_name: str, tag_content: str) -> dict:
        ...
        response = await self.bot.http_session.post(
            SITE_API_TAGS_URL, headers=self.headers, json=params
        )
        tag_data = await response.json()
    ...
    @command(
        name="tags.get()", aliases=["tags.get", "tags.show()", "tags.show", "get_tag"]
    )
    async def get_command(self, ctx: Context, tag_name: TagNameConverter = None):
        ...
        if _command_on_cooldown(tag_name):
            time_left = TAG_COOLDOWN - (
                time.time() - self.tag_cooldowns[tag_name]["time"]
            )
            log.warning(
    ...
    @command(
        name="tags.set()",
        aliases=[
            "tags.set", "tags.add", "tags.add()", "tags.edit", "tags.edit()", "add_tag"
        ],
    )
    async def set_command(
        self, ctx: Context, tag_name: TagNameConverter, tag_content: TagContentConverter
    ):
        """
    ...
    @command(
        name="tags.delete()",
        aliases=["tags.delete", "tags.remove", "tags.remove()", "remove_tag"],
    )
    async def delete_command(self, ctx: Context, tag_name: TagNameConverter):
        ...
        if tag_data.get("success") is True:
            ...
            log.debug(
                f"{ctx.author} tried to delete a tag called '{tag_name}', but the tag does not exist."
            )
            embed.colour = Colour.red()
            embed.title = tag_name
