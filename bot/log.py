...
def setup() -> None:
    """Set up loggers."""
    root_log = get_logger()
    _setup_file_logging(root_log)
    _setup_colored_logging(root_log)
...
def _setup_colored_logging(logger: logging.Logger) -> None:
    ...
    coloredlogs.install(level=core_logging.TRACE_LEVEL, logger=logger, stream=sys.stdout)
