...
class CodeJams(commands.Cog):
    ...
    @codejam.command()
    async def create(
        self, ctx: commands.Context, csv_file: t.Optional[str] = None
    ) -> None:
        """
        ...
        async with ctx.typing():
            if csv_file:
                async with self.bot.http_session.get(csv_file) as response:
                    if response.status != 200:
                        await ctx.send(
                            f"Got a bad response from the URL: {response.status}"
                        )
                        return
                ...
                raise commands.BadArgument(
                    "You must include either a CSV file or a link to one."
                )
            ...
            for row in reader:
                member = await get_or_fetch_member(
                    ctx.guild, int(row["Team Member Discord ID"])
                )

                if member is None:
                    log.trace(
                        f"Got an invalid member ID: {row['Team Member Discord ID']}"
                    )
                    continue

                teams[row["Team Name"]].append(
                    (member, row["Team Leader"].upper() == "Y")
                )

            team_leaders = await ctx.guild.create_role(
                name="Code Jam Team Leaders", colour=TEAM_LEADERS_COLOUR
            )

            for team_name, team_members in teams.items():
                await _channels.create_team_channel(
                    ctx.guild, team_name, team_members, team_leaders
                )

            await _channels.create_team_leader_channel(ctx.guild, team_leaders)
            await ctx.send(
                f"{Emojis.check_mark} Created Code Jam with {len(teams)} teams."
            )
    ...
    async def end(self, ctx: commands.Context) -> None:
        ...
        def predicate_deletion_emoji_reaction(
        ...
        try:
            await self.bot.wait_for(
                "reaction_add", check=predicate_deletion_emoji_reaction, timeout=10
            )
    ...
    async def _build_confirmation_message(
        ...
        def format_category_info(
            category: discord.CategoryChannel, channels: list[discord.abc.GuildChannel]
        ) -> str:
        ...
        deletion_details = "\n\n".join(
            format_category_info(category, channels)
    ...
    @commands.has_any_role(Roles.admins)
    async def move(
        self, ctx: commands.Context, member: Member, new_team_name: str
    ) -> None:
        ...
        if (
            old_team_channel.name == new_team_name
            or self.team_name(old_team_channel) == new_team_name
        ):
        ...
        await old_team_channel.set_permissions(
            member, overwrite=None, reason=f"Participant moved to {new_team_name}"
        )
    ...
    def jam_categories(guild: Guild) -> list[discord.CategoryChannel]:
        ...
        return [
            ...
            if category.name == _channels.CATEGORY_NAME
        ]

    @staticmethod
    def team_channel(
        guild: Guild, criterion: t.Union[str, Member]
        ...
        for category in CodeJams.jam_categories(guild):
            for channel in category.channels:
                if isinstance(channel, discord.TextChannel):
                    if (
                        # If it's a string.
                        criterion == channel.name
                        or criterion == CodeJams.team_name(channel)
                        # If it's a member.
